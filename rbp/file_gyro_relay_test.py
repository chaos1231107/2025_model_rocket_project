...
#create file 
for fname in  ["expected_angle4.txt", "gyro_value4.txt"]:
...
try:
    while True:
        ...
        rate_cond = abs(lpf_gyro_roll) > threshold_pitch_rate or abs(lpf_gyro_pitch) > threshold_pitch_rate
        
        if angle_cond and rate_cond:
            trigger_timer += dt
            if trigger_timer >= trigger_hold_time and not ejection_flag:
                print(">>Deployed!")
        ...
        with open("expected_angle4.txt", 'a') as f1:
            f1.write(
