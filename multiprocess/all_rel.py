...
# Stable calibration function (from second code)
...
def comm_process(roll, pitch, yaw, altitude, ejection_flag, lock):
    ...
    while True:
        ...
        with open('comm_data.txt', 'a') as f1:
            f1.write(
                f'Elapsed : {elapsed:.1f}, Roll : {roll.value:.1f}, Pitch: {pitch.value:.1f}, Yaw: {yaw.value:.1f}, Altitude: {altitude.value:.1f}\n'
            )
        time.sleep(0.07)
